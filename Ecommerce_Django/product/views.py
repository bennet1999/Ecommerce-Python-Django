...
def product_list(request,category,sub_category,sortby,min,max):
    ...
    if sub_category == 'None':
        products = Product.objects.filter(discount_price__range=(int(min),int(max)),category__name=category).order_by(sortby)
        ...
        products = Product.objects.filter(discount_price__range=(int(min),int(max)),sub_category__name=sub_category).order_by(sortby)
